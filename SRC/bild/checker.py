...
def ConsoleType(data):
	if data == b'\x81':
		return "[SD] System Debugger"
	if data == b'\x82':
		return "[DEX] Debug"
	if b'\x83' <= data <= b'\x8F':
		return "[CEX] Retail or [SHOP] Kiosk"
	if data == b'\xA0':
		return "[ARC] Arcade"
	else: 
		return "Unknow - check IDPS"
...
if __name__ == "__main__":
	...
	if flashType == "NOR":
		...
		idps = string2hex(getDatas(rawfiledata, 0x0002F070, 0xF)).upper()
		idps2 = string2hex(getDatas(rawfiledata, 0x000303D0, 0xF)).upper()
		PC = getDatas(rawfiledata, 0x002F075, 0x1)
		PC2 = getDatas(rawfiledata, 0x000303D5, 0x1)
	...
	if flashType == "NAND":
		...
		idps = string2hex(getDatas(rawfiledata, 0x00040870, 0xF)).upper()
		idps2 = string2hex(getDatas(rawfiledata, 0x00041BD0, 0xF)).upper()
		PC = getDatas(rawfiledata, 0x00040875, 0x1)
		PC2 = getDatas(rawfiledata, 0x00041BD5, 0x1)
	...
	if idps==idps2:
		print("idps : %s" % idps)
		print("Product Code : %s" % ConsoleType(PC))
	else:
		print("idps1: %s" % idps)
		print("idps2: %s" % idps2)
